    ...
    def make_dataset(self, task_id):
        ...
        for i, (name, c) in enumerate(zip(cls_names, cls_ids)):
            ...
            if self.args.dataset in ['cifar100', 'cifar10', 'mnist']:
                ...
                # targets_aux.append(np.zeros(len(idx), dtype=np.int) + self.seen_names.index(name))                    # EDIT: commented
                targets_aux.append(np.zeros(len(idx), dtype=int) + self.seen_names.index(name))                         # EDIT:[debug] changed np.int to int
...
def get_data(args):
    ...
    if args.dataset == 'mnist':
        ...
        test = ImageFolder(root=args.root + '/TinyImagenet/val', transform=test_transform)
